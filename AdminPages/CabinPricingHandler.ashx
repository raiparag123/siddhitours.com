<%@ WebHandler Language="C#" Class="CabinPricingHandler" %>
...
public class CabinPricingHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string a=context.Request.QueryString["CabinId"];
        // char[] arr = CabinTypeId.ToCharArray();
        int[] nums = Array.ConvertAll(a.Split(','), int.Parse);
         List <int> tmp = new List<int>(nums);
            tmp.RemoveAt(0);
        nums = tmp.ToArray();
        //int CabinId = Convert.ToInt32(context.Request.QueryString["CabinId"]);
        int[] CabinId = new int[] { 1, 2, 3, 4 };
        string status = "";
        ...
        try
        {
            if (context.Request.Files.Count > 0)
            {
                ...
                for (int i = 0; i < SelectedFiles.Count; i++)
                {
                    ...
                    int CabinTypeId = nums[i];
                    string FileName = context.Server.MapPath("~/Images/CabinImages/" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + PostedFile.FileName);
                    ...
                    {
                        compress = ConfigurationManager.AppSettings["filePath"] + "/CompressedCabinImages/" + DateTime.Now.ToString("yyyyMMddHHmmss") + "_" + PostedFile.FileName;
                        ...
                        SqlCommand cmd = new SqlCommand("SP_INSERT_CABIN_IMAGE", con);
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@CRUISEID", CruiseId);
                        cmd.Parameters.AddWithValue("@CABINID", CabinTypeId);
