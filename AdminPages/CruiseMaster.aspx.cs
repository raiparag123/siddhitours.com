...
public partial class AdminPages_CruiseMaster : System.Web.UI.Page
{
    ...
    public void bindData(int CruiseId)
    {
        ...
        try
        {
            ...
            intdetail.InnerText = dt.Rows[0]["Description"] != DBNull.Value ? dt.Rows[0]["Description"].ToString() : string.Empty;

            DataView dv2 = new DataView(ds.Tables[1]);
            dv2.RowFilter = "CruiseCabinId = 2";
            DataTable dt1 = dv2.ToTable();
            oceanviewdetails.InnerText = dt1.Rows[0]["Description"] != DBNull.Value ? dt1.Rows[0]["Description"].ToString() : string.Empty;

            DataView dv3 = new DataView(ds.Tables[1]);
            dv3.RowFilter = "CruiseCabinId = 3";
            DataTable dt2 = dv3.ToTable();
            balconydetail.InnerText = dt2.Rows[0]["Description"] != DBNull.Value ? dt2.Rows[0]["Description"].ToString() : string.Empty;

            DataView dv4 = new DataView(ds.Tables[1]);
            dv4.RowFilter = "CruiseCabinId = 4";
            DataTable d3 = dv4.ToTable();
            suitesdetail.InnerText = d3.Rows[0]["Description"] != DBNull.Value ? d3.Rows[0]["Description"].ToString() : string.Empty;
    ...
    public static string getCountryList()
    {
        ...
        try
        {
            ...
            SqlCommand cmd = new SqlCommand("SP_FE_PORTLIST", con);
            cmd.CommandType = CommandType.StoredProcedure;
    ...
    public static string InsertCruiseDetail(string Title, int days, int nights, string AlisaName, int Company, int Tax, int GAdult, int GChild, string Source, string Destination, string Inclusion, string Exclusion)
    {
        ...
        try
        {
            ...
            //cmd.Parameters.AddWithValue("@MAINPRICE", Convert.ToDecimal(MainPrice));
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataSet ds = new DataSet();
            sda.Fill(ds);
    ...
    [WebMethod]
    public static string InsertCabinPricing(int CruiseId, int [] CabinType, string [] Startdate, string [] Enddate, string [] price,string [] MainPrice)
    {
        string status = "";
        AdminPages_CruiseMaster adm = new AdminPages_CruiseMaster();
        SqlConnection con = new SqlConnection(conn);
        try
        {
            ...
            for (int i = 0; i < cnt;i++)
            {
                ...
                cmd.Parameters.AddWithValue("@MAINPRICE", Convert.ToDecimal(MainPrice[i]));
                SqlDataAdapter sda = new SqlDataAdapter(cmd);
                DataSet ds = new DataSet();
                sda.Fill(ds);
            }
    ...
    [WebMethod]
    public static string UpdateCabinPricing(int CruiseId, int[] CabinType, string[] Startdate, string[] Enddate, string[] price, int [] CabPriceId, string[] MainPrice)
    {
        string status = "";
        AdminPages_CruiseMaster adm = new AdminPages_CruiseMaster();
        SqlConnection con = new SqlConnection(conn);
        try
        {
            ...
            //SqlCommand cmd1 = new SqlCommand("SP_DELETE_CABIN_PRICE", con);
            //cmd1.CommandType = CommandType.StoredProcedure;
            //cmd1.Parameters.AddWithValue("@CRUISEID", CruiseId);
    ...
    [WebMethod]
    public static string DeleteCruise(int CruiseId)
    {
        string status = "";
        AdminPages_CruiseMaster adm = new AdminPages_CruiseMaster();
        SqlConnection con = new SqlConnection(conn);
        try
        {
            ...
            SqlCommand cmd = new SqlCommand("SP_DELETE_CRUISE", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@CRUISEID", CruiseId);
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable ds = new DataTable();
    ...
    public static string GetCruiseMasterDetail(int CruiseId)
    {
        ...
        try
        {
            if (HttpContext.Current.Session["cruise"] != null)
            {
                ...
                status += "~" + JsonConvert.SerializeObject(ds.Tables[5]);
            }
        }
        catch (Exception ex)
        {
